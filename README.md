...
1. **Create a repo:** on github.com → New repository → name it `code-writer-apk` → Create.
...
3. **Commit**, then open the **Actions** tab → click **Build Code Writer APK** →
   **Run workflow** → wait about 5–10 minutes.
4. **Download:** when it finishes, open the run → download the `code-writer-apk`
...
Done — the Code Writer is now a real installed app with your red handheld icon.
